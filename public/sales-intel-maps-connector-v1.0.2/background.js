/**
 * Background Service Worker for Sales Intel Chrome Extension (v1.0.2).
 * Handles authentication handshake, token refresh, batch lead imports,
 * AND single-owner discovery orchestration resilient to content-script re-injection.
 */
 ...
 * This state is SINGLE-OWNER and survives content script re-injections/recreations!
 */
...
function processNextCandidate() {
  ...
  // Send enrichment request to active tab
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // --- SI_CONTENT_READY ---
  // Sent by content script initialization/re-injection.
  if (message.type === "SI_CONTENT_READY") {
    console.log("[SI][LIFECYCLE][CONTENT_READY]");

    // Update active tab ID from sender
    ...
    if (discoveryState.active && discoveryState.activeCandidate) {
      ...
      // Re-trigger current candidate enrichment on newly attached content script
  ...
  if (message.type === "SI_START_DISCOVERY") {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      // Request content script to build discovery queue first
