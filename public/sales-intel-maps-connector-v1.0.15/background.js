...
function startNewRun(query, limit) {
  ...
  console.log(`[SI][RUN] START runId=${newRunId} query="${query || ""}" limit=${requestedLimit}`);
...
function getExportableLeads() {
  const ready = currentRun.readyLeads.filter(
    (lead) => lead.runId === currentRun.runId && lead.sourceQuery === currentRun.query
  );
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "SI_START_DISCOVERY") {
    chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
      ...
      startNewRun(mapsState.searchQuery, requestedLimit);
  ...
  if (message.type === "SI_DETAIL_READY") {
    ...
    if (runId !== currentRun.runId || (sourceQuery && sourceQuery !== currentRun.sourceQuery)) {
      console.log(`[SI][MSG] STALE response discarded: runId=${runId} currentRunId=${currentRun.runId}`);
