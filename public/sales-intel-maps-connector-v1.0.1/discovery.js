/**
 * Disposable DOM Content Worker for Google Maps (v1.0.2).
...
(function () {
  ...
  /** Builds candidate identity queue from Google Maps search results */
  ...
  /** Performs candidate click & detail panel extraction for active candidate */
  async function handleEnrichCandidate(candidate, index) {
    ...
    while (attempts < maxAttempts) {
      ...
      if (!isContextValid()) return; // Context invalidated by navigation
  ...
  if (isContextValid()) {
    ...
    chrome.runtime.sendMessage({ type: "SI_CONTENT_READY" }, () => {
      if (chrome.runtime.lastError) {
        /* ignore background setup */
      }
    });
