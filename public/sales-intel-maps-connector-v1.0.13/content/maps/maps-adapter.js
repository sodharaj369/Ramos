...
(function (root, factory) {
  ...
  function getRawCardElements() {
    ...
    for (const sel of cardSelectors) {
      try {
        ...
        for (const el of els) {
          // Strictly exclude elements inside the active detail panel
          if (el.closest && el.closest('div[role="main"], div.TIwYe, div.bJ181e')) {
            continue;
          }
          if (!seen.has(el)) {
            seen.add(el);
  ...
  function currentQuery() {
    ...
    const box =
      ...
      document.querySelector('input#searchboxinput, input[name="q"], input[aria-label*="Search" i]');
