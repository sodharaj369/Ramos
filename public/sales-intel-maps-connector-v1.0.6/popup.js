(function () {
  ...
  function applyMapsState(response) {
    el.mapsDot.className = "maps-dot green";
    el.mapsStatusTitle.textContent = "Google Maps detected";
    currentSearchQuery = response.searchQuery || response.query || null;
    if (currentSearchQuery) {
    ...
    if (response.running) {
      ...
      updateSummaryStats(response.stats, currentExtractedLeads.length);
    }
  }
  ...
  function updateCurrentBusiness(bizInfo) {
    ...
    setField(el.fieldAddress, bizInfo.hasAddress, "✓ Full Address", "— Address");
    setField(el.fieldPhone, bizInfo.hasPhone, "✓ Phone", "— Phone");
    setField(el.fieldWebsite, bizInfo.hasWebsite, "✓ Website", "— Website");
    setField(el.fieldRating, bizInfo.hasRating || bizInfo.hasReviews, "✓ Rating / Reviews", "— Rating");
    setField(el.fieldHours, bizInfo.hasOpeningStatus, "✓ Opening Status", "— Hours");
  }
  ...
  function startExtraction(onComplete) {
    const limit = 2; // Diagnostic limit = 2
