  ...
  return (
    <AppShell title="Settings" description="Account, providers and usage.">
      <div className="grid gap-5 items-start lg:grid-cols-2">
        <section className="rounded-lg border border-border bg-card p-4.5 shadow-xs">
        ...
        <section className="rounded-lg border border-border bg-card p-4.5 shadow-xs">
          <h2 className="text-sm font-semibold text-foreground">Provider usage</h2>
          {usage.isLoading ? (
        ...
        <section className="rounded-lg border border-border bg-card p-4.5 lg:col-span-2 shadow-xs">
          <h2 className="text-sm font-semibold text-foreground">Data providers</h2>
          ...
          {providers.isLoading ? (
            ...
            <ul className="mt-3 divide-y divide-border">
              {all.map((p) => (
                <li key={`${p.kind}-${p.id}`} className="py-2.5">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={
                        p.configured
                          ? "rounded-full bg-success/12 px-2 py-0.5 text-[10px] text-success border border-success/30"
                          : "rounded-full bg-warning/15 px-2 py-0.5 text-[10px] text-warning-foreground border border-warning/30"
                      }
                    >
        ...
        <section className="rounded-lg border border-border bg-card p-4.5 lg:col-span-2 shadow-xs">
          <h2 className="text-sm font-semibold text-foreground">Capabilities and current limits</h2>
          <ul className="mt-3 space-y-2 text-xs text-muted-foreground">
            <li>
              <span className="text-foreground font-medium">Background jobs</span> — discovery, import and
            ...
            <li>
              <span className="text-foreground font-medium">Built-in email verification</span> — syntax, domain
            ...
            <li>
              <span className="text-foreground font-medium">Demo lead source</span> — synthetic sample records
              for testing only. It is never selected automatically and results are prefixed with
              [DEMO].
            </li>
            <li>
              <span className="text-foreground font-medium">Account emails</span> — signup confirmation and
