...
class DiagnosticE2EEngine {
  ...
  logTrace(index: number, eventName: string, candidateName: string, extraStr = "") {
    ...
    console.log(`[RAMOS][E2E][${index}][${eventName}] name="${candidateName}" elapsedMs=${elapsedMs}${detail}`);
  }

  async runDiagnostic(liveCandidates: any[]) {
    // 1. Discovery phase
    console.log(`[RAMOS][E2E] START query="${this.currentRun.query}" limit=${this.currentRun.requestedLimit}`);
    
    // Select up to requested limit
    this.currentRun.candidates = liveCandidates.slice(0, this.currentRun.requestedLimit);
    console.log(`[RAMOS][E2E] DISCOVERED count=${liveCandidates.length} LIMITED count=${this.currentRun.candidates.length}`);
    ...
    console.log(
      `[RAMOS][E2E] DIAGNOSTIC_COMPLETE requested=${this.currentRun.requestedLimit} ready=${this.currentRun.readyLeads.length} failed=${this.currentRun.failedLeads.length} totalElapsedMs=${totalElapsed}`
    );
