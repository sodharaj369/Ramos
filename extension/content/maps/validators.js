...
(function (root, factory) {
  ...
  function isValidUrl(url) {
    ...
    if (GOOGLE_MAPS_URL_RE.test(trimmed)) return false;
  ...
  function validateAndCleanLead(rawLead) {
    ...
    cleaned.opening_status = sanitizeField(cleaned.opening_status);
    ...
    // Website — Accepts valid URLs or valid domain strings (rejecting Google Maps URLs)
    if (cleaned.website != null) {
      const w = cleanUnicode(cleaned.website);
      cleaned.website = isWebsiteText(w) ? w : null;
    } else {
      cleaned.website = null;
    }

    // URLs
    cleaned.booking_url = isValidUrl(cleaned.booking_url) ? cleaned.booking_url.trim() : null;
    cleaned.ordering_url = isValidUrl(cleaned.ordering_url) ? cleaned.ordering_url.trim() : null;
    cleaned.menu_url = isValidUrl(cleaned.menu_url) ? cleaned.menu_url.trim() : null;
    cleaned.source_url = isValidUrl(cleaned.source_url) ? cleaned.source_url.trim() : null;
